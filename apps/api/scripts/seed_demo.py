    ...
    recs = [
            ...
            evidence=[
                {
                    "type": "crawl",
                    "url": base + "/pricing",
                    "finding": "No title tag found in <head>",
                }
            ],
            ...
            evidence=[
                {
                    "type": "crawl",
                    "url": base + "/features",
                    "finding": "word_count=180 (threshold: 300)",
                }
            ],
        ...
        Recommendation(
            ...
            evidence=[
                {
                    "type": "analysis",
                    "finding": "No FAQ schema or Q&A structured content found in crawl",
                }
            ],
            affected_urls=[],
...
""",
        brief={
            "topic": "Engineering Sprints",
            "content_type": "blog",
            "target_keyword": "engineering sprint planning",
        },
        compliance_flags=[
            "Review all statistics before publishing",
            "No unverified claims found in this draft",
        ],
        risk_score=0.10,
    ...
    session.add(
        Report(
            ...
            sections=[
                {
                    "title": "Technical SEO",
                    "content": "2 critical issues: missing title on /pricing, 404 on /old-page.",
                },
                {
                    "title": "Content Pipeline",
                    "content": "1 blog post in review (low risk), 1 comparison page in draft (medium risk — competitor claims need verification).",
                },
                {"title": "Pending Approvals", "content": "1 item awaiting human approval."},
                {
                    "title": "AI Visibility",
                    "content": "Score: 42/100. Priority: add FAQ structured content to improve LLM citation readiness.",
                },
            ],
...
async def main() -> None:
    ...
    async with factory() as session:
        ...
        if existing.scalar_one_or_none():
            ...
            print(
                "   To reseed: docker compose down -v && docker compose up -d && make migrate && make seed"
            )
            return
        await seed(session)
