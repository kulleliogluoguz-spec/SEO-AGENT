...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "compliance_mode", sa.String(32), nullable=False
        ),  # "official_api" | "public_web" | "user_upload"
        sa.Column("display_name", sa.String(255), nullable=False),
        sa.Column("params", postgresql.JSONB, nullable=False, server_default="{}"),
        sa.Column(
            "credentials_ref", sa.String(512)
        ),  # Reference to encrypted secret, NOT stored here
    ...
    op.create_index(
        "ix_source_documents_workspace_source", "source_documents", ["workspace_id", "source_type"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "category", sa.String(64), server_default="general"
        ),  # "pain_point" | "solution" | "competitor" | "trend" | etc.
    ...
    op.create_table(
        ...
        sa.Column(
            "relevance_score", sa.Float, server_default="0.0"
        ),  # Relevance to this workspace's brand
        ...
        sa.Column(
            "source_types", postgresql.ARRAY(sa.Text), server_default="{}"
        ...
        sa.Column(
            "status", sa.String(32), server_default="active"
        ),  # "active" | "declining" | "expired"
        sa.Column("metadata", postgresql.JSONB, server_default="{}"),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "channels", postgresql.ARRAY(sa.Text), server_default="{}"
        ),  # Where they hang out
        sa.Column("keywords", postgresql.ARRAY(sa.Text), server_default="{}"),
    ...
    op.create_table(
        ...
        sa.Column("description", sa.Text, server_default=""),
        sa.Column(
            "defining_signals", postgresql.JSONB, server_default="{}"
        ),  # keywords, communities, behaviors
        ...
        sa.Column(
            "channel_distribution", postgresql.JSONB, server_default="{}"
        ),  # e.g. {reddit: 0.4, twitter: 0.3}
        sa.Column(
            "persona_ids", postgresql.ARRAY(postgresql.UUID(as_uuid=True)), server_default="{}"
        ),
        sa.Column(
            "trend_ids", postgresql.ARRAY(postgresql.UUID(as_uuid=True)), server_default="{}"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(32), server_default="pending"
    ...
    op.create_table(
        ...
        sa.Column(
            "crawl_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("crawls.id", ondelete="SET NULL"),
        ),
