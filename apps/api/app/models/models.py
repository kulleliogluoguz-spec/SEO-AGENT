...
class Workspace(Base):
    ...
    autonomy_level: Mapped[int] = mapped_column(Integer, default=AutonomyLevel.DRAFT_ONLY)
...
class Site(Base):
    ...
    status: Mapped[str] = mapped_column(
        SAEnum(SiteStatus, name="site_status"), default=SiteStatus.PENDING
    )
    product_summary: Mapped[str | None] = mapped_column(Text)
    category: Mapped[str | None] = mapped_column(String(100))
    icp_summary: Mapped[str | None] = mapped_column(Text)
    settings: Mapped[dict] = mapped_column(JSONB, default=dict)
    ...
    recommendations: Mapped[list["Recommendation"]] = relationship(back_populates="site")
...
class CrawlPage(Base):
    ...
    meta_tags: Mapped[dict] = mapped_column(JSONB, default=dict)
    internal_links: Mapped[list] = mapped_column(JSONB, default=list)
    external_links: Mapped[list] = mapped_column(JSONB, default=list)
    issues: Mapped[list] = mapped_column(JSONB, default=list)
    screenshot_path: Mapped[str | None] = mapped_column(String(512))
    rendered_fallback: Mapped[bool] = mapped_column(Boolean, default=False)
    ...
    __table_args__ = (
        Index("ix_crawl_pages_crawl_id", "crawl_id"),
        UniqueConstraint("crawl_id", "url", name="uq_crawl_page_url"),
    )
...
class Recommendation(Base):
    ...
    affected_urls: Mapped[list] = mapped_column(JSONB, default=list)
    proposed_action: Mapped[str | None] = mapped_column(Text)
    rollback_plan: Mapped[str | None] = mapped_column(Text)
    impact_score: Mapped[float] = mapped_column(default=0.5)
    effort_score: Mapped[float] = mapped_column(default=0.5)
    confidence_score: Mapped[float] = mapped_column(default=0.5)
    urgency_score: Mapped[float] = mapped_column(default=0.5)
    priority_score: Mapped[float] = mapped_column(default=0.5, index=True)
    ...
    __table_args__ = (
        Index("ix_recommendations_site_status", "site_id", "status"),
        Index("ix_recommendations_priority", "priority_score"),
    )
...
class Approval(Base):
    ...
    policy_flags: Mapped[list] = mapped_column(JSONB, default=list)
    status: Mapped[str] = mapped_column(
...
class AgentRun(Base):
    ...
    status: Mapped[str] = mapped_column(
        SAEnum(AgentRunStatus, name="agent_run_status"), default=AgentRunStatus.RUNNING
    )
