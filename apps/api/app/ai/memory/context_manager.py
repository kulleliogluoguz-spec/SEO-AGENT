...
class AIContext:
    ...
    def to_system_context(self) -> str:
        ...
        if self.competitors.competitors:
            comp_names = [
                c.get("name", c.get("url", "unknown")) for c in self.competitors.competitors[:5]
            ]
...
class ContextManager:
    ...
    async def update_site_context(self, site_id: str, **updates: Any) -> None:
        for _key, contexts in self._contexts.items():
    ...
    def clear_cache(self, workspace_id: str | None = None) -> None:
        if workspace_id:
            self._contexts = {
                k: v for k, v in self._contexts.items() if not k.startswith(f"{workspace_id}:")
            }
        else:
