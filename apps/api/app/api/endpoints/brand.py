...
from app.intelligence.niche_data import get_intelligence, get_dashboard_overview, infer_niche
...
def _data_mode(profile: dict) -> str:
    ...
    has_real_name = bool(profile.get("brand_name") and profile["brand_name"].lower() not in ("acme", "demo", "example"))
...
def _provenance(mode: str, field: str) -> str:
    ...
    if mode == "demo":
        return "demo"
    user_provided_fields = {"brand_name", "instagram_handle", "website_url", "category", "target_audience", "geography", "business_goal", "description"}
    ...
    if field in observed_fields:
        return "observed"
    estimated_fields = {"cpm_range", "cpc_range", "cac_estimate", "roas_estimate", "budget_estimate"}
...
async def get_audience(current_user=Depends(get_current_user)) -> dict:
    ...
    segments = [
        {**s, "segment_name": s.get("segment_name") or s.get("name", "")}
...
def _compute_growth_stage(profile: dict) -> dict:
    ...
    if followers < 1000 or post_count < 50:
        ...
        next_milestone = f"{max(0, 20 - outcome_count)} more labeled outcomes to reach optimizing stage"
    ...
    cold_start_mix = {
        ...
        "travel":    {"inspirational": 0.45, "educational": 0.25, "lifestyle": 0.20, "engagement": 0.10},
        "ecommerce": {"product": 0.40, "social_proof": 0.30, "educational": 0.20, "engagement": 0.10},
        "creator":   {"behind_scenes": 0.35, "educational": 0.30, "inspirational": 0.25, "engagement": 0.10},
        "b2b":       {"educational": 0.50, "case_study": 0.25, "thought_leadership": 0.15, "engagement": 0.10},
        "wellness":  {"educational": 0.40, "motivational": 0.30, "lifestyle": 0.20, "engagement": 0.10},
        "general":   {"educational": 0.35, "inspirational": 0.30, "product": 0.25, "engagement": 0.10},
    ...
    return {
        ...
        "posting_cadence_per_week": 5 if stage == "cold_start" else (7 if stage == "growing" else None),
        "niche": niche,
    }
...
async def update_growth_metrics(
    ...
    allowed_fields = {"followers", "post_count", "engagement_rate", "reach", "labeled_outcome_count"}
...
class ActivationRequest(BaseModel):
    ...
    primary_goal: str = "grow_followers"   # grow_followers | drive_traffic | increase_sales | build_awareness
...
async def activate_brand(
    ...
    intel = get_intelligence(profile)
    niche = intel.get("niche", "general")
    overview = get_dashboard_overview(profile)
    ...
    for signal in signals[:3]:
        campaign_angles.append({
            ...
            "suggested_budget_usd": min(
                (payload.monthly_budget_usd or 300) / 30,
                50
            ),
            "platform": "meta",
        })
    ...
    recommended_mode = (
        "assisted" if (has_budget and has_social) else "manual"
    )
    ...
    return {
        ...
        "growth_plan_summary": {
            ...
            "monthly_budget_usd": payload.monthly_budget_usd,
            "top_opportunity": intel.get("recommendations", [{}])[0].get("title", "") if intel.get("recommendations") else "",
        },
