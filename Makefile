...
shell-qdrant: ## Open Qdrant web UI in browser
	@echo "$(GREEN)Qdrant dashboard: http://localhost:6333/dashboard$(RESET)"

# ─── Local AI Models ──────────────────────────────────────────────────────────
models-pull: ## Pull Tier 1 local models (CPU-compatible, ~6GB)
	@echo "$(GREEN)Pulling Tier 1 models (CPU-compatible)...$(RESET)"
	docker compose exec ollama ollama pull nomic-embed-text
	docker compose exec ollama ollama pull qwen3:8b
	@echo "$(GREEN)Tier 1 models ready.$(RESET)"

models-pull-tier2: ## Pull Tier 2 models (consumer GPU, ~15GB)
	$(MAKE) models-pull
	@echo "$(GREEN)Pulling Tier 2 models...$(RESET)"
	docker compose exec ollama ollama pull qwen3:30b-a3b
	@echo "$(GREEN)Tier 2 models ready.$(RESET)"

models-pull-tier3: ## Pull Tier 3 models (production GPU, ~60GB)
	$(MAKE) models-pull-tier2
	@echo "$(GREEN)Pulling Tier 3 models (large — may take time)...$(RESET)"
	docker compose exec ollama ollama pull qwen3:235b-a22b
	docker compose exec ollama ollama pull llama4:scout
	@echo "$(GREEN)Tier 3 models ready.$(RESET)"

models-list: ## List available Ollama models
	docker compose exec ollama ollama list

models-health: ## Check AI model health
	@curl -sf http://localhost:11434/api/tags | python3 -m json.tool 2>/dev/null || echo "$(YELLOW)Ollama not running$(RESET)"
	@curl -sf http://localhost:6333/readyz && echo "$(GREEN)Qdrant: healthy$(RESET)" || echo "$(YELLOW)Qdrant not running$(RESET)"
