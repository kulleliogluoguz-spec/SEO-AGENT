...
async def generate_content(
    payload: GenerateContentRequest, user: CurrentUser = Depends(get_current_user)
):
...
async def repurpose_content(
    payload: RepurposeContentRequest, user: CurrentUser = Depends(get_current_user)
):
...
async def create_content_item(
    payload: ContentItemCreate, user: CurrentUser = Depends(get_current_user)
):
    ...
    return [
        {
            ...
            "body": "Here's what nobody tells you about growth...",
...
async def list_pending_approvals(
    decision: ApprovalDecision | None = Query(default=ApprovalDecision.PENDING),
    user: CurrentUser = Depends(get_current_user),
):
    return [
        {
            ...
            "channel": "instagram",
            "body": "Ready to level up your marketing game?...",
            "decision": "pending",
            "risk_score": 0.12,
            "risk_level": "low",
            "compliance_check": {
...
async def batch_schedule(
    payload: BatchScheduleRequest, user: CurrentUser = Depends(get_current_user)
):
    ...
    now = datetime.utcnow()
    errs = [
        {"index": i, "error": "past date"} for i, t in enumerate(payload.schedule_times) if t < now
    ]
    return {
        "scheduled": len(payload.content_item_ids) - len(errs),
        "failed": len(errs),
        "errors": errs,
    }
...
async def publish_content(
    ...
    return {
        "content_item_id": content_item_id,
        "channel": ch,
        "daily_remaining": rem - (1 if result.get("published") else 0),
        **result,
    }
...
async def create_ad_campaign(
    payload: AdCampaignCreate, user: CurrentUser = Depends(get_current_user)
):
...
async def get_performance_feedback(user: CurrentUser = Depends(get_current_user)):
    return await marketing_service.get_performance_feedback(
        workspace_id=user.workspace_id,
        metrics=[
            {"impressions": 5000, "likes": 200, "comments": 30, "shares": 15},
...
async def generate_hooks(
    ...
    return {
        "hooks": await marketing_service.generate_hooks(user.workspace_id, topic, channel, count)
    }
...
async def generate_hashtags(
    ...
    return {
        "hashtag_strategy": await marketing_service.generate_hashtags(
            user.workspace_id, topic, channel, niche
        )
    }
...
async def get_channel_strategy(
    industry: str = Query(default="saas"), user: CurrentUser = Depends(get_current_user)
):
    return {
        "strategy": await marketing_service.get_channel_strategy(
            user.workspace_id, industry=industry
        )
    }
